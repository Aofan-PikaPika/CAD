...
namespace BLL.ComputeUnits.F2
{
    //计算支架立杆轴向力设计值N的公式
    //单位：千牛
    public class F_NWind : Formula<double>
    {
        ...
        public F_NWind(double NG1K, double NG2K, double NQK)
        {
            this.NG1K = NG1K;
            this.NG2K = NG2K;
            this.NQK = NQK;
        ...
        {
            _targetValue = 1.2 * (NG1K + NG2K) + 0.9*1.4 * NQK;
            _isComputed = true;
        ...
        {
            if (_isComputed)
                return "1.2×(" + NG1K.ToString("#0.00") + "+" + NG2K.ToString("#0.00") + ")+0.9×1.4×" + NQK.ToString("#0.00")+"="+_targetValue.ToString("#0.00");
            else
                return "";
        }
    }
